...
public class Scanner {
    ...
    public Scanner(BufferedReader input) {
        ...
        word = new String[]{"begin", "call", "const", "do", "else", "end", "for", "if", "odd",
                "procedure", "read", "return", "step", "then", "until", "var", "while", "write"};
        ...
        wsym[11] = Symbol.returnsym;
        wsym[12] = Symbol.stepsym;
        wsym[13] = Symbol.thensym;
        wsym[14] = Symbol.untilsym;
    ...
    private void matchOperator() {
        ...
        switch (ch) {
            ...
            case '*':      //乘等（改）
                getch();
                if (ch == '=') {
                    sym = Symbol.timebecomesym;
                    getch();
                } else sym = Symbol.times;
                break;
            case '/':      //除等（改）
                getch();
                if (ch == '=') {
                    sym = Symbol.slashbecomesym;
                    getch();
                } else sym = Symbol.slash;
                break;
            case '|':
                getch();
                if (ch == '|') {
